...
/**
 * Die Klasse berechnet ein Intensitaetsbild mittels einer refokussierten
 * Gradientenechosequenz. Geraeteparameter sind Repititions- und Echozeit, sowie
 * der Flipwinkel.
 * 
 ...
 */
public class GERefocused extends Pulsesequence {
	...
	protected void addSequenceParameterToImage() {
		...
		ResultIP.setSequence("GE-refocused");
	...
	public void calculate() {
		...
		for (int x = 0; x < PDMatrix.length; x++) {
			...
			for (int y = 0; y < PDMatrix[0].length; y++) {
				...
				IntensityMatrix[x][y] = (int) ((PDMatrix[x][y]) * (sinangel) / (1 + (T1Matrix[x][y] / t2x) - ((T1Matrix[x][y] / t2x) - 1) + cosangel));
